...
describe('crypto', () => {
  it('should throw when params are empty', () => {
    expect(() => decrypt('', '')).toThrow('messages clientSecret 不可为空');
  });

  it('should decrypt messages successfully', () => {
    ...
    const expected = '{"appId":110,"buyerId":123456,"buyerPhone":"13800138000","env":"PROD","kdtId":160,"orderNo":"E201905060000001","payTime":1557138032000,"price":42,"skuIntervalText":"7","skuVersionText":"试用版","status":20,"type":"APP_SUBSCRIBE"}';
    expect(decrypt(messages, clientSecret)).toBe(expected);
  });
});
